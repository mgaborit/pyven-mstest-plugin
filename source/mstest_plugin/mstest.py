...
from mstest_plugin.trx_parser import TRXParser

class MSTest(Process):

    def __init__(self, cwd, name, path, filename, arguments):
        super(MSTest, self).__init__(cwd, name)
        self.duration = 0
        self.type = 'mstest'
        self.path = path
    ...
    def process(self, verbose=False, warning_as_error=False):
        ...
        if os.path.isdir(self.cwd):
            ...
            if os.path.isfile(os.path.join(self.cwd, self._format_report_name())):
                os.remove(os.path.join(self.cwd, self._format_report_name()))
            ...
            if returncode != 0:
                ...
                if os.path.isfile(os.path.join(self.cwd, self._format_report_name())):
                    ...
                    msg = 'Could not find TRX report --> '+os.path.join(self.path, self._format_report_name())
    ...
    def report_properties(self):
        properties = []
        properties.append(('Test file', os.path.join(self.path, self.filename)))
        properties.append(('Duration', str(self.duration) + ' seconds'))
        return properties
        
    def _format_report_name(self):
        return self.filename+'-'+self.type+'.trx'
    ...
    def _format_call(self, clean=False):
        call = ['MSTest', '/resultsfile:' + self._format_report_name(), '/testcontainer:' + self.filename]
